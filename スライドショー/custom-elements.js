...
class CHTMLImageSeq extends ExtensionNode {
	...
	resized(targets) {
		...
		const	vn = this.vn;
		let i,k, css, cvar, target, computed,scale,datum,rect;
		...
		while (target = targets[++i]) {
			...
			scale = computed.getPropertyValue('--image-seq-scale'),
			
			rect = target.getBoundingClientRect();
			for (k in rect) typeof rect[k] === 'number' && (css += `${vn}-rect-${k}:${rect[k]}px;`);
			
			for (k in this.data) {
				css += `${vn}-${k}:${datum = this.data[k]};`
				scale === '' || isNaN(parseInt(datum)) || (css += `${vn}-${k}-scaled:calc(${datum} * ${scale});`);
			}
			
			cvar = this.parseCvar(target, rect, computed, scale);
			for (k in cvar) css += `${vn}-${k}:${cvar[k]};`;
	...
	set() {
		...
		const	targets = document.querySelectorAll(this.ifor),
				items = this.querySelectorAll(':scope > c-ii'),
				l = items.length,
				data = {
					urls: [],
					'max-width': 0,
		...
		let i,i0,k,k0,ii, item,img;
		
		i = ii = -1, this.data = data;
		while (item = items[++i]) {
			
			data.urls.push(item.urls),
			data['max-width'] < item.maxWidth && (data['max-width'] = item.maxWidth),
			data['max-height'] < item.maxHeight && (data['max-height'] = item.maxHeight),
			
			data[`${k = item.id || i}-max-width`] = `${item.maxWidth}px`,
			data[`${k}-max-height`] = `${item.maxWidth}px`,
			data[`${k}-accumulative-width`] = `${item.accumulativeWidth = data['total-width']}px`,
			data[`${k}-accumulative-height`] = `${item.accumulativeHeight = data['total-height']}px`,
			data[`${k}-total-width`] = `${item.totalWidth}px`,
			data[`${k}-total-height`] = `${item.totalHeight}px`,
			data['total-length'] += data[`${k}-length`] = item.images.length,
			i0 = -1;
			while (img = item.images[++i0])	data[`${k}-${i0}-url`] = `url("${img.src}")`,
														data[`${k}-${i0}-accumulative-width`] =
															`${item.accumulativeWidth + img.accumulativeWidth}px`,
														data[`${k}-${i0}-accumulative-height`] =
															`${item.accumulativeHeight + img.accumulativeHeight}px`,
														data[`${k}-${i0}-width`] = `${img.width}px`,
														data[`${k}-${i0}-height`] = `${img.height}px`,
														
														++ii,
														data[`${k0 = item.id || ii}-url`] = data[`${k}-${i0}-url`],
														data[`${k0}-accumulative-width`] = data[`${k}-${i0}-accumulative-width`],
														data[`${k0}-accumulative-height`] = data[`${k}-${i0}-accumulative-height`],
														data[`${k0}-width`] = data[`${k}-${i0}-width`],
														data[`${k0}-height`] = data[`${k}-${i0}-height`];
			
			data['total-width'] += item.totalWidth,
			data['total-height'] += item.totalHeight;
		...
		data['max-height'] += 'px',
		data['total-width'] += 'px',
		data['total-height'] += 'px';
		
	};
	parseCvar(node, rect, computed, scale) {
		
		const	items = this.querySelectorAll(':scope > c-ii'), data = this.data, cvar = this.cvar;
		let	i,l,i0,k,k0, cvarRaw,cvarRawScaled,cvarValue,cvarValueScaled,cssValue,placeholder,
				simplexCvarIndex,isSimplex,indicesRaw,indicesRawValues,indices,imgs,img, index,itemIndex,
				placeholderName,placeholderValue,asNum,placeholderValueScaled, parsed;
		...
		while (items[++i]) {
			i0 = -1;
			while (items[i].images[++i0]) (imgs[imgs.length] = items[i].images[i0]).itemIndex = i;
		}
		
		l = imgs.length, cvarValue = [], cvarValueScaled = [], parsed = {};
		for (k in cvar) {
			
			if ((i = isSimplex = CHTMLImageSeq.rxSb.exec(k0 = k)) !== null) {
			...
			while (img = imgs[++i]) {
				...
				cvarRaw = cvarRawScaled = cvar[k], index = i, itemIndex = img.itemIndex;
				while (placeholder = CHTMLImageSeq.rxCb.exec(cvarRaw)) {
					...
					if (indicesRaw = CHTMLImageSeq.rxSb.exec(placeholder[1])) {
						...
						placeholderName = placeholder[1].replace(indicesRaw[0], '');
					...
					(asNum = placeholderName[placeholderName.length - 1] === '*') &&
					...
					switch (placeholderName.toLowerCase()) {
						...
						placeholderValue = indicesRaw ?
							placeholderName === 'U' ? items[itemIndex].urls.join() : imgs[index].url : data.urls.join();
						break;
						case 'w': case 'h':
						...
						placeholderValue = indicesRaw ?
							(`${placeholderName === 'W' ?	items[itemIndex].totalWidth :
							placeholderName === 'w' ?	imgs[index].width :
							placeholderName === 'H' ?	items[itemIndex].totalHeight :
																imgs[index].height}px`) :
							data[`total-${placeholderName.toLowerCase() === 'w' ? 'width' : 'height'}`];
						break;
						case '!w': case '!h':
						// 画像の最大幅、高さを示す。大文字、小文字の指定にかかわらず、c-ii に指定された画像の中から最大幅ないし高さと置換する。
						// 添え番を指定しない場合、c-is が内包する c-ii に指定されたすべての画像の最大幅ないし高さと置換する。
						placeholderValue = indicesRaw ?
							`${items[itemIndex][`max${placeholderName[1].toLowerCase() === 'w' ? 'Width' : 'Height'}`]}px` :
							data[`max-${placeholderName[1].toLowerCase() === 'w' ? 'width' : 'height'}`];
						break;
						case '~w': case '~h':
						...
						placeholderValue = indicesRaw ? (placeholderName[1] === 'w' || placeholderName[1] === 'h') ?
							`${imgs[index][`accumulative${placeholderName[1] === 'w' ? 'Width' : 'Height'}`]}px` :
							`${items[itemIndex][`accumulative${placeholderName[1] === 'W' ? 'Width' : 'Height'}`]}px` : 0;
						break;
						...
						placeholderValue = `${rect[CHTMLImageSeq.placeholderDict[placeholderName[1]]]}px`;
						break;
						...
						case 'i': placeholderValue = placeholderValue === 'I' ? itemIndex : index; break;
						case 'l': placeholderValue = placeholderValue === 'L' ? items.length : l; break;
						default:
						...
						placeholderValue = indicesRaw ? placeholderName[1][0] === ':' ?
							items[index][placeholderName[1].slice(1)] : imgs[index][placeholderName[1]] :
							this[placeholderName[1]];
					}
					
					(scale === '' || isNaN(parseInt(placeholderValue))) ||
						(placeholderValueScaled = `calc(${placeholderValue} * ${scale})`),
					
					asNum && (placeholderValue = placeholderValue.replace(/px/g, '')),
					
					cvarRaw = cvarRaw.replace(placeholder[0], placeholderValue);
					placeholderValueScaled && (cvarRawScaled = cvarRawScaled.replace(placeholder[0], placeholderValueScaled));
					
				}
				
				cvarValue[cvarValue.length] = cvarRaw,
				cvarRaw === cvarRawScaled || (cvarValueScaled[cvarValueScaled.length] = cvarRawScaled);
	...
	*/
	...
	static bound = {
		...
		defined() {
			...
			const fetched = () => ++i0 === l && (this.set(), this.emit('load-image-sequence')),
					items = this.querySelectorAll(':scope > c-ii'),
					l = items.length;
			let i,i0;
			
			i = -1, i0 = 0;
			while (items[++i]) items[i].available.then(fetched).catch(error => alert(error));
